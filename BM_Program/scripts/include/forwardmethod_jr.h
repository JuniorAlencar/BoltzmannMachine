...
//METROPOLIS---------------------------------------------------------------------------------------
void metropolis_bm(Rede &r, VecDoub_IO &av_s, VecDoub_IO &av_ss,
                   const int t_eq, const int t_step, const int relx,
                   const int rept, const double beta,
                   std::mt19937 &generator)
{
    ...
    std::uniform_int_distribution<int> dist_spin(0, r.n - 1);
    ...
    for (int j = 0; j < rept; j++)
    {
        for (int i = 0; i < t_eq + t_step; i++)
        {
            ...
            s_flip = dist_spin(generator);             // substitui rand() % r.n
            dE = delta_E(r, s_flip);
            ...
            {
                p = dist_real(generator);              // substitui rand()/RAND_MAX
