...
def load_data2(file):
    ...
    pattern = pattern_properties2(file)
...
def load_data(file):
    ...
    props = ["correlation", "magnetization", "sisj", "sisjsk", "triplet"]
    file_t = [f"{comp_folder}/{i}/" for i in props]
    files = [f'Pij_exp_ising_{pattern[0]}_err_j_{pattern[1]}_err_h_{pattern[2]}.dat', 
             f'mag_exp_ising_{pattern[0]}_err_j_{pattern[1]}_err_h_{pattern[2]}.dat', 
             f'sisj_exp_ising_{pattern[0]}_err_j_{pattern[1]}_err_h_{pattern[2]}.dat',
             f'sisjsk_exp_ising_{pattern[0]}_err_j_{pattern[1]}_err_h_{pattern[2]}.dat', 
             f'Tijk_exp_ising_{pattern[0]}_err_j_{pattern[1]}_err_h_{pattern[2]}.dat']
    filenames = [file_t[i] + files[i] for i in range(len(files))]
