...
//#include "./include/parallel_tempering_kernels.h"
...
//-----------------------------------------------------------------------------
	...
	// Generator
	std::mt19937 gen(seed);  // apenas uma vez fora do loop
	...
	{	
		...
		if(method == "metropolis")
			...
			parallel_tempering_cuda_multi(n_replicas, T_min, T_max,
				bm_av_s, bm_av_ss,
				t_eq, t_step, relx, rept,
				n, mean, sigma,
				type, H,
				energy_per_replica, temperatures,
				swap_ratio, gen
			);

			std::cout << "Swap acceptance rate: " << swap_ratio << std::endl;
